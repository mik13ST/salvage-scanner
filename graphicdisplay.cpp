...
/*The DOGM128-6 boasts a resolution of 128x64 pixels. 8 lines of 21 characters*/
#if defined(LCD_PCD8544)

	#define LCD_SCLK_PIN 2			// SCK
  #define LCD_DIN_PIN  3			// MOSI
  #define LCD_DC_PIN   4			// A0
  #define LCD_CS_PIN   5			// CS
  #define LCD_RST_PIN  6			// RST
  #define BACKLIGHT    7

	#define LENGTHCHARS	 14
	#define LENGTHLINES	 6
	#define BUFFSIZE		 84		// 6x14

	GraphicDisplay::GraphicDisplay(uint8_t sck, uint8_t mosi, uint8_t cs, uint8_t a0, uint8_t reset)
		             :U8GLIB_PCD8544(LCD_SCLK_PIN, LCD_DIN_PIN, LCD_CS_PIN, LCD_DC_PIN, LCD_RST_PIN){
	}
	void GraphicDisplay::lcd_data( unsigned char Data) {
    _symbuffer[_row*LENGTHCHARS+_column] = Data;
    if( _column < (LENGTHCHARS-1)) _column++;
	}
	void GraphicDisplay::lcd_clear( void ) {
	...
	void GraphicDisplay::print(const char *Str)
	{
    for( int i = _row*LENGTHCHARS+_column; i < _row*LENGTHCHARS+LENGTHCHARS; i++ ) {
      _symbuffer[i] = (byte)*Str++;
      _column++;
    }
		this->update();
  ...
  #define X_CENTRAL 56
  #define Y_CENTRAL 32
  #define X_OFFSET (X_CENTRAL-ICON_WIDTH/2)
  #define Y_OFFSET (Y_CENTRAL-ICON_HEIGHT/2)
  ...
  void GraphicDisplay::update(void)
  {
    ...
    do {
      ...
      // symbol buffer
      char buf[LENGTHCHARS+1];
      for( int k=0; k <LENGTHLINES; k++) {
        for( int i=0; i <LENGTHCHARS; i++) {
          buf[i] = _symbuffer[k*LENGTHCHARS+i];
        }
        buf[LENGTHCHARS] = 0;
        this->drawStr((u8g_uint_t)0,(u8g_uint_t)(k*8+8),(const char*)buf);
        //Serial.println(buf);
      ...
      switch (_comp) {
        ...
        case COMPONENT_DIODE:
					this->drawTriangle(X_OFFSET + 12, Y_CENTRAL - 8, X_OFFSET + 12, Y_CENTRAL + 8, X_OFFSET + 20, Y_CENTRAL);
					this->drawBox(X_OFFSET + 20, Y_CENTRAL - 8, 2, 17);
					this->drawHLine(X_OFFSET + 3,Y_CENTRAL, 26);
          break;
        case COMPONENT_ZENER:
          this->drawHLine( X_OFFSET + 3, Y_CENTRAL, 4);
